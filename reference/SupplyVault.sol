    ...
    ReentrancyGuard
{
    ...
    constructor(
        ...
        ERC20(
            _name.orElse(
                string("Tarot ")
                    .append(ERC20(address(_underlying)).symbol())
                    .append(" Supply Vault")
            ),
            _symbol.orElse(
                string("t").append(ERC20(address(_underlying)).symbol())
            )
        )
    {
    ...
    function _addBorrowable(address _address) private {
        ...
        require(
            address(borrowable.underlying()) == address(underlying),
            "V:INVLD_UL"
        );
    ...
    function addBorrowable(address _address)
        external
        override
        onlyOwner
        nonReentrant
    ...
    function addBorrowables(address[] calldata _addressList)
        external
        override
        onlyOwner
        nonReentrant
    ...
    function indexOfBorrowable(IBorrowable borrowable)
        public
        view
        override
        returns (uint256)
    ...
    function removeBorrowable(IBorrowable borrowable)
        external
        override
        onlyOwner
        nonReentrant
    ...
    function disableBorrowable(IBorrowable borrowable)
        external
        override
        onlyOwner
        nonReentrant
    ...
    function enableBorrowable(IBorrowable borrowable)
        external
        override
        onlyOwner
        nonReentrant
    ...
    function unwindBorrowable(IBorrowable borrowable, uint256 borrowableAmount)
        external
        override
        onlyOwner
    {
        ...
        uint256 borrowableAmountAsUnderlying = borrowable.underlyingValueOf(
            borrowableAmount
        );
        require(borrowableAmountAsUnderlying > 0, "V:U_ZERO");

        uint256 available = Math.min(
            borrowable.myUnderlyingBalance(),
            underlying.balanceOf(address(borrowable))
        );
        require(borrowableAmountAsUnderlying <= available, "V:NEED_B");

        uint256 underlyingBalanceBefore = underlying.balanceOf(address(this));
        IERC20(address(borrowable)).safeTransfer(
            address(borrowable),
            borrowableAmount
        );
        borrowable.redeem(address(this));
        uint256 underlyingAmount = underlying.balanceOf(address(this)).sub(
    ...
    function updatePendingStrategy(
        ISupplyVaultStrategy _newPendingStrategy,
        uint256 _notBefore
        ...
        if (address(_newPendingStrategy) == address(0)) {
            ...
            require(
                address(_newPendingStrategy) != address(0),
                "V:INVLD_STRAT"
            );
    ...
    function updateStrategy() external override onlyOwner nonReentrant {
        ...
        require(
            block.timestamp < pendingStrategyNotBefore + GRACE_PERIOD,
            "V:TOO_LATE"
        );
    ...
    function updateFeeBps(uint256 _newFeeBps)
        external
        override
        onlyOwner
        nonReentrant
    {
        require(
            _newFeeBps >= MIN_FEE_BPS && _newFeeBps <= MAX_FEE_BPS,
            "V:INVLD_FEE"
        );
    ...
    function updateFeeTo(address _newFeeTo)
        external
        override
        onlyOwner
        nonReentrant
    ...
    function updateReallocateManager(address _newReallocateManager)
        external
        override
        onlyOwner
        nonReentrant
    ...
    function getBorrowableEnabled(IBorrowable borrowable)
        external
        view
        override
        returns (bool)
    ...
    function getBorrowableExists(IBorrowable borrowable)
        external
        view
        override
        returns (bool)
    ...
    function getTotalUnderlying()
        ...
        override
        nonReentrant
        returns (uint256 totalUnderlying)
    {
    ...
    function _enterWithToken(address _tokenAddress, uint256 _tokenAmount)
        private
        returns (uint256 share)
    {
        ...
        if (_tokenAddress == address(underlying)) {
            ...
            underlyingAmount = IBorrowable(_tokenAddress).underlyingValueOf(
                _tokenAmount
            );
        } else {
        ...
        returns (uint256 underlyingAmount)
    {
        ...
        uint256 needUnderlyingAmount = _share.mul(totalUnderlyingBefore).div(
            totalShares
        );
        uint256 haveUnderlyingAmount = underlying.balanceOf(address(this));
        if (needUnderlyingAmount > haveUnderlyingAmount) {
            needUnderlyingAmount = needUnderlyingAmount.sub(
                haveUnderlyingAmount
            );
...

        {
            ...
            if (belowCheckpoint) {
                ...
                // C = C * (S - s) / S;
                newCheckpointBalance = checkpointBalance
                    .mul(totalShares.sub(_share))
                    .div(totalShares);
            } else {
    ...
    function leaveInKind(uint256 _share) external override nonReentrant {
        ...
        bool sentSomething = _transferShareOfToken(
            address(underlying),
            _share,
            totalShares
        ) > 0;
    ...
    function reallocate(uint256 _share, bytes calldata _data)
        external
        override
        nonReentrant
    {
        require(
            msg.sender == owner() || msg.sender == reallocateManager,
            "V:NOT_AUTHORIZED"
        );
        ...
        uint256 underlyingAmount = _share.mul(totalUnderlyingBefore).div(
            totalShares
        );
    ...
    function allocateIntoBorrowable(
        ...
        uint256 borrowableAmount = borrowable.balanceOf(address(this)).sub(
            borrowableBalanceBefore
        );

        emit AllocateBorrowable(
            address(borrowable),
            underlyingAmount,
            borrowableAmount
        );
    ...
    function deallocateFromBorrowable(
        IBorrowable borrowable,
        ...
        if (totalUnderlying > checkpointBalance) {
            if (feeTo != address(0)) {
                ...
                if (fee > 0) {
                    uint256 totalShares = totalSupply();
                    // X = F * S / (U - F)
                    uint256 feeShares = fee.mul(totalShares).div(
                        totalUnderlying.sub(fee)
                    );
                    if (feeShares > 0) {
