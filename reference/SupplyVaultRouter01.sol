...
contract SupplyVaultRouter01 is ISupplyVaultRouter01 {
    ...
    function _enterWithAlloc(ISupplyVault vault, uint256 underlyingAmount)
        private
        returns (uint256 share)
    {
    ...
    function enterWithAlloc(ISupplyVault vault, uint256 underlyingAmount)
        external
        override
        returns (uint256 share)
    {
    ...
    function enterWithAllocETH(ISupplyVault vault)
        external
        payable
        override
        checkETH(vault)
    ...
    function _leave(ISupplyVault vault, uint256 share)
        private
        returns (IERC20 underlying, uint256 underlyingAmount)
    {
    ...
    function leave(ISupplyVault vault, uint256 share)
        external
        override
    ...
    modifier checkBorrowable(ISupplyVault vault, IBorrowable borrowable) {
        require(
            address(vault.underlying()) == borrowable.underlying(),
            "SVR: WRONG_BORROWABLE"
        );
        _;
    }
}
