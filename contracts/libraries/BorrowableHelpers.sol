...
library BorrowableHelpers {
    ...
    // function borrowableValueOf(IBorrowable borrowable, uint256 underlyingAmount)
    //     internal
    //     returns (uint256)
    // {
    ...
    //         }
    //         utilizationRate_ = (nextBalance == 0)
    //             ? 0
    //             : totalBorrows.mul(RATE_SCALE).div(nextBalance);
    //     }
    //     uint256 kinkUtilizationRate = borrowable.kinkUtilizationRate(); // gas savings
    //     if (utilizationRate_ <= kinkUtilizationRate) {
    //         borrowRate_ = borrowable.kinkBorrowRate().mul(utilizationRate_).div(
    //                 kinkUtilizationRate
    //             );
    //     } else {
    ...
    //             uint256 overUtilization = utilizationRate_
    //                 .sub(kinkUtilizationRate)
    //                 .mul(RATE_SCALE)
    //                 .div(RATE_SCALE.sub(kinkUtilizationRate));
    //             borrowRate_ = borrowRate_.mul(overUtilization);
    //         }
    //         borrowRate_ = borrowRate_
    //             .add(RATE_SCALE)
    //             .mul(borrowable.kinkBorrowRate())
    //             .div(RATE_SCALE);
    //     }
    ...
    //     )
    // {
    //     (borrowRate_, utilizationRate_) = getNextBorrowRate(
    //         borrowable,
    //         depositAmount,
    //         withdrawAmount
    //     );
